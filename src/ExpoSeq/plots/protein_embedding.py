...
class Plot_Embedding:
    def __init__(self,ax, sequencing_report,model_choice, list_experiments,strands,add_clone_size, batch_size, pca_components, perplexity, iterations_tsne, font_settings, legend_settings):
        self.ax = ax
        Transformer = TransformerBased(choice = model_choice)

        sequences, selected_rows, selected_rows = Transformer.filter_sequences(sequencing_report, batch_size=batch_size, experiments = list_experiments, region_of_interest="aaSeqCDR3", )
    ...
    def add_legend(self, list_experiments, legend_settings):
        plt.legend(labels=list_experiments,
                   title = "Sample Names",
                **legend_settings)
