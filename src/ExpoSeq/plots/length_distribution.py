...
def length_distribution_single(fig,ax, sequencing_report, sample, font_settings, region_of_interest):
    batch = sequencing_report[sequencing_report["Experiment"] == sample]
    length = batch[region_of_interest].str.len()
...
def length_distribution_multi(fig, sequencing_report, samples,num_cols, font_settings, region_of_interest, test_version = False,):
    if samples == "all":
