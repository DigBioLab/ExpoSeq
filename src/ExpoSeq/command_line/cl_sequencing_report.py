...
Report = SequencingReport(
    sequencing_report=parser.tsv_dir
)  # first extract original sequencing_report (raw data)
Report.prepare_seq_report(
    region_string=parser.region,
...
avail_regions = list(
    orig_report_cols.intersection(possible_regions)
)  # get available aaSeq cols from your raw data
for (
    region
) in (
    avail_regions
):  # loop through all available regions and merge the tidied data in one report -> you will have Na's then
    region = region.replace("aaSeq", "")
    RegionReport = SequencingReport(sequencing_report=parser.tsv_dir)
    RegionReport.prepare_seq_report(
        region_string=parser.region,
        length_threshold=parser.length_threshold,
        min_read_count=parser.min_read_count,
        remove_gaps=parser.remove_gaps,
    ...
    all_sequences_report = pd.concat(
