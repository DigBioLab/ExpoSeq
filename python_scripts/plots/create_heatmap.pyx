...
cpdef np.ndarray [double, ndim = 2]loop_morosita_horn(nested_seq,
    ...
    cdef int first_cluster, second_cluster, seq_index, index_seq2, start
    cdef float xi, yi, product_xy, denmuerator_morosita_horn, numerator_morosita_horn, morosita_horn_index, sum_xy,
    ...
    heatmap_morosita_horn = np.zeros([37, 37])
    start = 0
    for first_cluster in range(0, 37):
        ...
        for second_cluster in range(start, 37):
            ...
            for seq_index in range(0, len(sample_one)):
                ...
                length_seq = len(seq)
                if seq in sample_two and seq not in used_seq: # oherwise if seq2 > seq1 they will be counted twice  and seq not in used_seq
